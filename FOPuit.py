...
# Calcul de l'échelle pour le potentiel (statique)
v_scaled = V * (np.max(np.abs(psi)**2) * 0.8) / np.max(V)

# Tracer le potentiel V (ligne violette pointillée)
ax1.plot(listeX, v_scaled, lw=1.5, color='purple', linestyle='--', label='Potentiel V(x)')
...
def animate(frame):
    t = frame * step  # Indice temporel réel
